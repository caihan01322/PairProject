...
var document = window.document;
	...
	函数DOMEval（代码，节点，文档）{
		...
		script.text =代码；
		如果（节点）{
			为（我在preservedScriptAttributes中）{
...
getText = Sizzle.getText = function（elem）{
	...
	如果（！nodeType）{
		...
		//对元素使用textContent
		//删除innerText用法以保持新行的一致性（jQuery＃11153）
...
函数matcherFromGroupMatchers（elementMatchers，setMatchers）{
	var bySet = setMatchers.length> 0，
		...
		superMatcher = function（种子，上下文，xml，结果，最外层）{
			...
			//没有元素匹配器，也没有种子。
			//将初始字符串“ 0”递增为“ i”，则允许“ i”仅在该字符串中保留字符串
...
jQuery.extend（{
	...
	递延：function（func）{
		...
		jQuery.each（元组，函数（i，元组）{
			...
			deferred [元组[0] +“ With”] = list.fireWith;
		}）；
