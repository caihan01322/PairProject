...
[代码规范](https://blog.csdn.net/anwoliao6541/article/details/101807180)

### python代码布局

- 缩进
  - 每个缩进级别统一采用Tab键。多行填写代码时，可以采用空格以达到垂直对齐。

- 每行最多字符数
  - 每行最长300个字符，不采用换行中断一行代码。
...
- main函数放于文件底部。
- 数据库连接暂不划分python文件。[对于flask框架不熟悉，未来会做更改]
- 库的调用写在python文件最上方。

- 每个接口函数代码前添加函数接口返回值和传递值。